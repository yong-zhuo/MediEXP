...
namespace Keyboard
{
    public class KeyboardManager : MonoBehaviour
    {
        ...
        public void ResetMode(){
            // This is for external use by ResetCameraPos, Do not use this method internally
            mode = 0;
            OffQuestionsAudio(false);
        }
        private void Awake()
        {
        ...
        private void OnEnterPress(){
            if(mode == 0){
                if(gameSpeech.getInitialPlay()){
                    gameSpeech.OffAudio();
                }
                else{
                    resetCameraPos.OffAudio();
                }
            }
            OffQuestionsAudio(true);
            GetComponents<AudioSource>()[mode].Play();
            mode = (mode + 1) % AUDIOSRCS;
        }

        private void OffQuestionsAudio(bool isSelf){
            if(isSelf){
                if(mode == 0){
                    GetComponents<AudioSource>()[AUDIOSRCS - 1].Stop();
                }
                else{
                    GetComponents<AudioSource>()[mode - 1].Stop();
                }
                return;
            }
            GetComponents<AudioSource>()[mode].Stop();
        }
